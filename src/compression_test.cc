...
TEST(Compression, DecompressEmpty) {
    EXPECT_TRUE(check_decompress_buffer(
        COMPRESSION_BSLZ4_HDF5,
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"s, 1, ""s));
    EXPECT_TRUE(check_decompress_buffer(
        COMPRESSION_LZ4_HDF5,
        "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"s, 0, ""s));
}
...
TEST(Compression, DecompressErrorLz4BlockSizeOverflow) {
    const auto ENCODED = "\x00\x00\x00\x00\xff\xff\xff\xfe\xff\xff\xff\xfe\x00\x00\x00\x01\x00"s;
    char buffer[1];
    EXPECT_EQ(
        compression_decompress_buffer(COMPRESSION_LZ4_HDF5, buffer, 0xffffffff,
